...
class BluetoothAgent extends dbus.interface {
  constructor(manager) {
  ...
  async RequestAuthorization(devicePath) {
    const deviceMac = this.manager.extractMacFromPath(devicePath);
    const peripheralMac = this.manager.peripheral?.address.toUpperCase();
...
const registerBluetoothAgent = async (manager) => {
  ...
  try {
    ...
    bus.export('/org/bluez/agent', agent);
    debug(`[Bluetooth Agent] Agent exported at ${objectPath}`);

    // Get AgentManager interface
    const bluezManager = await bus.getProxyObject('org.bluez', '/');
    const agentManager = bluezManager.getInterface('org.bluez.AgentManager1');
    ...
    setupBluetoothAgentCleanup(bus, bluezManager, objectPath);
  } catch (err) {
...
module.exports = registerBluetoothAgent;
