...
const debug = require('debug')('bluetooth-manager:debug');
...
module.exports = class BluetoothManager {
  ...
  readValueAndWait(uuid, firstByte, delay = -1, timeout = 3000) {
    const characteristic = this.characteristicsMap.get(uuid);
    const name = UUIDToName[uuid] || 'Unknown UUID';
  ...
  async registerBluetoothAgent() {
    ...
    try {
      ...
      const agent = {
        ...
        RequestConfirmation: async (devicePath, passkey) => {
          const deviceMac = BluetoothManager.extractMacFromPath(devicePath);
          const peripheralMac = this.peripheral?.address.toUpperCase();
        ...
        AuthorizeService: async (device, uuid) => {
          debug(`[Bluetooth Agent] Authorizing service ${uuid} for ${device}`);
        },
        RequestAuthorization: async (devicePath) => {
          const deviceMac = this.extractMacFromPath(devicePath);
          const peripheralMac = this.peripheral?.address.toUpperCase();
      ...
      const rootObj = await bus.getProxyObject('org.bluez', '/');
      await rootObj.export(objectPath, agent, 'org.bluez.Agent1');

      debug(`[Bluetooth Agent] Agent exported at ${objectPath}`);

      // Get AgentManager interface
      const manager = rootObj.getInterface('org.bluez.AgentManager1');

      // Register the agent
      await manager.RegisterAgent(objectPath, 'NoInputNoOutput');
      debug('[Bluetooth Agent] Bluetooth agent registered successfully');

      // Set as default agent
      await manager.RequestDefaultAgent(objectPath);
      debug('[Bluetooth Agent] Default agent set');

      // Setup cleanup on exit
      BluetoothManager.setupBluetoothAgentCleanup(bus, manager, objectPath);
    } catch (err) {
  ...
  // Cleanup function
  static async cleanupBluetoothAgent(bus, manager, objectPath) {
    try {
  ...
  static setupBluetoothAgentCleanup(bus, manager, objectPath) {
    const shutdown = async () => {
      debug('\nShutting down...');
      await BluetoothManager.cleanupBluetoothAgent(bus, manager, objectPath);
      process.exit(0);
    };
    ...
    process.on('exit', () => {
      // Final safety net
      BluetoothManager.cleanupBluetoothManager(manager, objectPath).catch(() => {});
    });
  }
};
