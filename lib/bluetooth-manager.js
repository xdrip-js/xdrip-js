...
function jealousPromise(peripheral, executor) {
...
module.exports = class BluetoothManager {
  ...
  didDiscover(peripheral) {
    ...
    peripheral.once('connect', async () => {
      ...
      try {
          const mtu = await peripheral.writeMTU(247);
          debug(`MTU successfully changed to ${mtu}`);
      } catch (err) {
          debug('Failed to set MTU:', err);
      }
  ...
  writeValueAndWait(value, uuid, timeout = 3000) {
