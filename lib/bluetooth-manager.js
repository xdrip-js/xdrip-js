...
module.exports = class BluetoothManager {
  constructor(delegate, deviceAddr = null) {
  ...
  async didDiscoverDevice(device) {
    ...
    let isConnected = await device.isConnected().catch(() => false);
    if (isConnected) {
    ...
    try {
      ...
      const connectPromise = device.connect().then(
        async () => {
          if (cancelled) {
            // connect won the race but we already decided to cancel
            await device.disconnect().catch(() => {});
            throw new Error('Connect completed after cancel');
          }
        },
      );
      ...
      const timeoutPromise = new Promise((_, reject) => {
        connectTimer = setTimeout(
          async () => {
            ...
            try {
              await device.disconnect(); // abort the in-progress Connect
            } catch (e) {
              // ignore – device may not be fully registered yet
            }
      ...
      await this.didDeviceConnect();
    } catch (err) {
      debug(`→ ERROR: ${err.message || err}`);

      isConnected = await device.isConnected().catch(() => false);
      // also check isConnecting to make sure that we have not proceeding
      // into didDeviceConnect from the attempt that timed out because
      // it could have proceeded while awaiting in the line above
  ...
  async didDeviceConnect() {
    ...
    const undiscovered = await this.discoverCharacteristicsAndWait(service);

    if (undiscovered.size === 0) {
      this.delegate.isReady(this.deviceAddr);
  ...
  handleDisconnect() {
    ...
    this.device = null;
    this.isConnecting = false;
    // checkForDevices may be stuck, so allow another to start since we disconnected
    this.checkingDevices = false;
  ...
  async isDevicePaired(adapter = null) {
    if (!this.deviceAddr) {
      debug('Unable to determine if device is paired: no deviceAddr set');
  ...
  async unpairDevice(adapter = null) {
    if (!this.deviceAddr) {
      debug('Unable to unpairDevice: no deviceAddr set');
      return Promise.resolve(false);
    }
