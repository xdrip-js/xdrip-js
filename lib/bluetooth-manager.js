...
BluetoothManager.prototype.writeValueAndWaitForNotification = function(value, uuid, firstByte, timeout = 2000) {
  ...
  //
    characteristic.once('data', function callback(data) {
      debug('got data ' + data.toString('hex'));
      if ((!firstByte) || (data[0] === firstByte)) {
...
// bluetooth operation
class JealousPromise extends Promise {
  constructor(executor) {
    if (outstandingPromise) {
      throw new Error('bluetooth busy');
    }
    super(executor);
    outstandingPromise = this;
    // set outstanding promise to null on both resolution and rejection
    // seems to cause infinite recursion for some reason???
    this.then(
