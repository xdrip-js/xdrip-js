...
module.exports = class BluetoothManager {
  ...
  didDiscover(peripheral) {
    ...
    peripheral.once('disconnect', () => {
      ...
      if (!this.discoverSuccess && (this.discoverFailures < 12)) {
  ...
  writeValueAndWait(value, uuid, timeout = 2000) {
  ...
  readValueAndWait(uuid, firstByte, timeout = 2000) {
    const characteristic = this.characteristicsMap.get(uuid);
    return new JealousPromise(this.peripheral, (resolve, reject) => {
  ...
  setNotifyEnabledAndWait(enabled, uuid, timeout = 2000) {
  ...
  waitForNotification(uuid, firstByte, timeout = 2000) {
    const characteristic = this.characteristicsMap.get(uuid);
    return new JealousPromise(this.peripheral, (resolve, reject) => {
  ...
  writeValueAndWaitForNotification(value, uuid, firstByte, timeout = 2000) {
    const characteristic = this.characteristicsMap.get(uuid);
    return new JealousPromise(this.peripheral, (resolve, reject) => {
      characteristic.once('data', (data) => {
        debug(`Rx ${data.toString('hex')}`);
