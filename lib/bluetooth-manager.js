...
const registerBluetoothAgent = require('./bluetooth-agent');
...
function scanForPeripheral() {
  ...
  // Have to continuously kick off scanning rasberry pi
  // only scans a very brief time every 30 seconds
  setInterval(() => {
    noble.stopScanning();
    setTimeout(() => {
      noble.startScanning(serviceUUIDs, true);
    }, 200);
  }, 10000); // restart every 30 seconds
}
...
module.exports = class BluetoothManager {
  constructor(delegate) {
    ...
    noble.on('warning', (message) => debug(`Noble warning: ${message}`));

    //registerBluetoothAgent(this);
  }
