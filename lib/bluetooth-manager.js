    ...
    return this.btQueue.enqueue({
      ...
      executor: async ({ addCleanup }) => {
        ...
        try {
          ...
          debug(`Device state before bonding - Paired: ${props.Paired}, Connected: ${props.Connected}`);

          if (props.Paired === true) {
            debug(`Already bonded with ${address}`);
            return true;
          }

          debug(`Starting bond with ${address}...`);
          await device.Pair(); // ← This is the createBond equivalent

          debug(`Bonding with ${address} successful`);
          return true;
        } catch (err) {
          debug(`Bonding failed for ${address}: ${err.message}`);

          // Helpful diagnostics
          if (err.message.includes('interface not found') || err.message.includes('No such interface')) {
            debug('Device object not fully ready in BlueZ. Try scanning first or waiting a moment.');
          }
