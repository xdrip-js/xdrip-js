...
const debugDiscover = require('debug')('bluetooth-manager:discover');
...
module.exports = class BluetoothManager {
    ...
    this.discoveryWatchdog = setInterval(async () => {
      try {
        ...
        await adapter.startDiscovery().catch((err) => {
          debugDiscover(`Watchdog startDiscovery failed: ${err.message}`);
        });

        debugDiscover('Discovery watchdog restarted scan');
      } catch (err) {
        debugDiscover(`Discovery watchdog error: ${err.message}`);
      }
    }, intervalMs);
  ...
  async didDiscoverDevice(device, retry = false) {
    ...
    debug(`Found device: ${name} | RSSI: ${rssi} | retry=${retry} | addr=${addr}`);
