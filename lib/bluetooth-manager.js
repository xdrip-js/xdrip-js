...
module.exports = class BluetoothManager {
  ...
  didDiscover(peripheral, retry = false) {
    ...
    const connectedHandler = (connectStartTime) => {
    ...
    const attemptConnect = () => {
      ...
      const connectTimeout = setTimeout(() => {
        ...
        if (this.peripheral.state === 'connected') {
          debug(`→ SUCCESS: Connected after ${Date.now() - connectStartTime}ms`);
          connectedHandler(connectStartTime);
        }
      }, 5000);
  ...
  async pairDevice(adapter) {
    ...
    return this.btQueue.enqueue({
      operationName: `pairDevice ${address}`,
      ...
      executor: async ({ addCleanup }) => {
        ...
        try {
          ...
          debug(`Failed to pair ${address}: ${err.message}`);
          return false;
        }
      },
    });
