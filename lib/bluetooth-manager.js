...
BluetoothManager.prototype.didDisconnect = function(peripheral) {
  ...
  // TODO: remove this commented out stuff if the bind line works
  // setTimeout(() => {
  //   console.log("scanning again");
  //   this.scanForPeripheral();
  // }, 60000); // TODO: consider scanning again 4.5 minutes after last connect (could save power?)
...
class JealousPromise {
  constructor (executor) {
    ...
    return outstandingPromise.then(
      value => {
        console.log('promise resolved, setting to null');
        outstandingPromise = null;
        return value;
      },
      reason => {
        console.log('promise rejected (' + reason + '), setting to null');
        outstandingPromise = null;
        throw reason;
      }
    );
