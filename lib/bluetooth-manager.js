...
module.exports = class BluetoothManager {
  constructor(delegate) {
    ...
    this.queue = new SerialOperationQueue(50);
  ...
  createDisconnectHandler(operationName) {
  ...
  writeValueAndWait(value, uuid, timeout = 5000) {
    ...
    return this.queue.enqueue({
      ...
      onStart: this.createDisconnectHandler(
  ...
  setNotifyEnabledAndWait(enabled, uuid, timeout = 3000) {
    ...
    return this.queue.enqueue({
      ...
      executor: (resolve, reject) => {
        ...
        // Timeout protection
        const timer = setTimeout(() => {
          reject(new Error(`Timeout enabling notifications for ${name}(${uuid})`));
        }, timeout);
  ...
  waitForNotification(uuid, firstByte, timeout = 3000) {
    ...
    return this.queue.enqueue({
      ...
      onStart: (resolve, reject) => {
        const disconnectHandler = () => {
          reject(new Error(
            `Peripheral disconnected during waitForNotification on ${name}(${uuid})`,
          ));
        };
        ...
        return () => {
          this.peripheral.removeListener('disconnect', disconnectHandler);
          characteristic.removeListener('data', dataHandler);
        };
      },
      ...
      executor: (resolve, reject) => {
        ...
        // Return cleanup for executor-owned resources (the timer).
        return () => {
          clearTimeout(timer);
        };
      },
  ...
  writeValueAndWaitForNotification(value, uuid, firstByte, timeout = 3000) {
    ...
    return this.queue.enqueue({
      ...
      onStart: (resolve, reject) => {
        const disconnectHandler = () => {
          reject(new Error(
            `Peripheral disconnected during writeValueAndWaitForNotification to ${name}(${uuid})`,
          ));
        };
    ...
    return this.queue.enqueue({
      ...
      executor: async (resolve, reject) => {
        ...
        try {
          ...
          resolve(paired.value);
        } catch (err) {
          debug(`Error during query if device paired ${address}: ${err.message}`);
          reject(err);
        } finally {
          bus.disconnect();
        }
