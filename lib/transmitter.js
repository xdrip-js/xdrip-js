...
const SessionStartTxMessage = require('./messages/session-start-tx-message');         // 0x26 - g5
const G6SessionStartTxMessage = require('./messages/g6-session-start-tx-message');    // 0x26 - g6 
    ...
    promise = promise.then(() => {
      switch (message.type) {
        case "StartSensor":
          if (this.g6_sensor) {
            // for now
	    sensorSerialCode = "0000";
            const sessionStartMessage = new G6SessionStartTxMessage(messageTimeInDexSeconds, sensorSerialCode);
          } else {
