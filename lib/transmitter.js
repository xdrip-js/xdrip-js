...
function processMessages(messages, uuid, syncDate, timeMessage) {
  ...
  while (messages.length) {
    ...
    promise = promise.then(() => {
      switch (message.type) {
        case "BackStartSensor":
          const sessionBackStartMessage = new SessionStartTxMessage(messageTimeInDexSeconds-2*60*60);
          return manager.writeValueAndWaitForNotification(sessionBackStartMessage.data, uuid)
