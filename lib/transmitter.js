...
async function authenticate2() {
  ...
  this.manager.setupNotification(uuid.Authentication);
  ...
  while (true) {
    // null for second argument because any first byte is acceptable
    // eslint-disable-next-line no-await-in-loop
    const data = await this.manager.readValueAndWait(uuid.Authentication, null);
    debug(`Received Authentication ${data.toString('hex')}`);

    if (Plugin.bondNow(data)) {
      debug('Creating bond!');
      return;
    }

    if (((this.plugin.state !== Plugin.RequestAuth) || (data.length > 5)) && this.plugin.receivedResponse(data)) {
      // eslint-disable-next-line no-await-in-loop
      await doNext.call(this);
    }
  }
...
module.exports = class Transmitter extends EventEmitter {
  ...
  shouldConnect(peripheral) {
    ...
    if (shortTxId(this.id) && (name.substr(0, 4) === 'DXCM')) {
