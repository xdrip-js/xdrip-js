...
function fullAuthenticate(uuid) {
  ...
  .then(data => {
    const status = new AuthStatusRxMessage(data);
    if ((status.authenticated === 1) && (status.bonded === 1)) {
      debug('peripheral authenticated and bonded');
      return;
    } else {
      debug('peripheral not fully authenticated: ' + data.toString('hex'));
      // I think we need the keepalive even if we're not doing full auth
      ...
      .then(() => manager.readValueAndWait(uuid, 15000));
      // then check for 050101 ...
    }
  });
}
