    ...
    while (!bonded) {
      ...
      if (currentValueHex !== previousValueHex) {
        ...
        if (bondNow && !bonded) {
          ...
          debug('Discovering services again');
          const service = await this.manager.discoverServicesAndWait();

          if (service) {
            debug(`Services discovered: ${service.uuid}`);

            debug('Discovering characteristics again');
            const success = await this.manager.discoverCharacteristicsAndWait(service);

            if (success) {
              debug('Discovering characteristics again success');
            } else {
              debug('Discovering characteristics again failed');
            }
          }
        }
