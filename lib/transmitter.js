  ...
  while (messages.length) {
    ...
    promise = promise.then(() => {
      switch (message.type) {
        case "StartSensor":
          if (this.g6_sensor) {
            var sessionStartMessage = new this.SessionStartTxMessage(messageTimeInDexSeconds, message.sensorSerialCode);
          } else {
            var sessionStartMessage = new this.SessionStartTxMessage(messageTimeInDexSeconds);
          }
