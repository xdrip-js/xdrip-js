...
Transmitter.prototype.calibrate = function(glucose) {
  ...
  // return new Promise(resolve => {
  //   calibrations.push({time, glucose, resolve});
  // });
...
function control() {
  ...
  .then(data => {
    ...
    return processCalibrations(uuid, syncDate, timeMessage).then(debug)


// // THE NEW WAY TO DO CALIBRATIONS
//     while (calibrations.length) {
//       const calibration = calibrations.shift();
//       // TODO: pass in the correct time (from calibration.time)
//       const calibrationMessage = new CalibrateGlucoseTxMessage(calibration.glucose, timeMessage.currentTime);
...
//       });
//     }
//     // END OF THE NEW WAY TO DO CALIBRATIONS
...
function processCalibrations(uuid, syncDate, timeMessage) {
  if (calibrationPending) {
    const calibrationMessage = new CalibrateGlucoseTxMessage(
      calibrationPending.glucose,
      timeMessage.currentTime - Math.floor((syncDate - calibrationPending.date)/1000)
    );
    return manager.writeValueAndWaitForNotification(calibrationMessage.data, uuid)
