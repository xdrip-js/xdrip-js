...
async function processMessages(messages, uuid, syncDate, timeMessage) {
  while (messages.length) {
    ...
    // esdlint-disable-next-line no-await-in-loop
...
async function control(messages) {
  ...
  let timeMessage;
  try {
    const data = await this.manager.writeValueAndWaitForNotification(new TransmitterTimeTxMessage().data, uuid);
  ...
  let glucoseMessage;
  try {
    const data = await this.manager.writeValueAndWaitForNotification(new GlucoseTxMessage(this.g6Transmitter).data, uuid);
