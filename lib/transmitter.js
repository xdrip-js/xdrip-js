...
function Transmitter(id, getMessages) {
  ...
  if (id_first == "8") {
      ...
      const SessionStartTxMessage = require('./messages/g6/session-start-tx-message'); // 0x26 - g6 
  } else {
      const SessionStartTxMessage = require('./messages/g5/session-start-tx-message'); // 0x26 - g5
  }
    ...
    promise = promise.then(() => {
      switch (message.type) {
	    ...
	    sensorSerialCode = "0000";
            const sessionStartMessage = new SessionStartTxMessage(messageTimeInDexSeconds, sensorSerialCode);
          } else {
