...
function control(messages) {
  ...
  .then(data => {
    ...
    return processCalibrations.call(this, messages, uuid, syncDate, timeMessage)
...
function processCalibrations(messages, uuid, syncDate, timeMessage) {
  var promise = Promise.resolve();
  while (messages.length) {
    const calibration = messages.shift();
    promise = promise.then(function() {
      ...
      .then(message => {
        this.emit('messageProcessed', {time: calibration.date});
