...
const CalibrationTxMessage = require('./messages/calibration-tx-message');
...
const GlucoseRxMessage = require('./messages/glucose-rx-message');
...
Transmitter.prototype.isReady = function() {
  ...
  authenticateUsingNotification.call(this)
...
function authenticateUsingNotification() {
  // TODO: consider what to do if the device is already authenticated?
  // e.g. try to read from Auth characteristic, if 050101 skip the rest?
  // or maybe some other tests instead?
  console.log('in authenticate');
...
//
  ...
  .then(() => {
    ...
    const message = new BondRequestTxMessage();
    // 0x01 here just as placeholder
    return manager.writeValueAndWaitForNotification(message.data, uuid, '0x01', 15000);
  })
  // TODO: the program doesn't wait here for the pairing request to be accepted
  // before moving on to the control step - need to fix.
  .then(data => {
...
function control() {
  ...
  .then(data => {
    ...
    console.log('message: ' + timeMessage.toString('hex'));

////////////// CALIBRATION /////////
    // console.log('creating calibration tx message');
    ...
    // .then(data => {
    //   console.log('calibrationRxMessage: ' + data.toString('hex'));

///////////// SESSION START //////////
    // console.log('creating session start tx message');
    ...
    // .then(data => {
    //   console.log('sessionStartRxMessage: ' + data.toString('hex'));
