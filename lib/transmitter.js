  ...
  if (p.length === 2) {
    ...
    if (data !== null) {
      ...
      for (let i = 0; i < len; i += 20) {
        ...
        this.manager.writeValueNoWait(bytesToSend, uuid.ExtraData);
    ...
    if (cmd !== null) {
      this.manager.writeValueAndWait(p[0]);
    }
  } else if (p.length === 1) {
    debug(`ready to receive data! Plugin in state ${this.plugin.stateToName(this.plugin.state)}`);
  } else if (p.length === 3) {
    /*
     * I believe this is used for saving the bond information
...
async function authenticate2() {
  ...
  this.manager.setupNotification(uuid.ExtraData, async (data) => {
    if (this.plugin.receivedData(data)) {
      await doNext().call(this);
    }
  });
  ...
  while (this.plugin.state !== this.plugin.GetData) {
    const data = this.readValueAndWait(uuid.Authentication, null /* any first byte acceptable */);
    debug(`Received Authentication ${data.toString('hex')}`);

    if (this.plugin.bondNow(data)) {
      debug('Creating bond!');
    }
...
module.exports = class Transmitter extends EventEmitter {
  constructor(id, getMessages = (() => []), altBt = false, sharedKey = null, macAddress = null) {
    ...
    if (shortTxId(this.id)) {
      ...
      this.plugin.setPersistance(2, sharedKey);
    }
  ...
  shouldConnect(peripheral) {
    ...
    if (shortTxId(this.id) && (name.substr(-4) === 'DXCM')) {
  ...
  async isReady(macAddress) {
    ...
    try {
      ...
      if (!shortTxId(this.id)) {
        status = await authenticate.call(this);
      } else {
        if (macAddress !== this.savedMacAddress) {
          ...
          this.plugin.setPersistance(2, null);
        }

        status = await authenticate2.call(this);
        ...
        if (key) {
          ...
          this.emit('sharedKey', key);
          this.emit('macAddress', macAddress);
        }
      }
