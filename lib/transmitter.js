...
// NOTE: it's possible there is no 0x35: just 0x33, responding to both 0x32 and 0x34
// const CalibrationRxMessage = require('./messages/calibration-rx-message');          // 0x35
...
function authenticate() {
  ...
  // TODO: this throw is caught by the one later - ideally we want the then chain to stop here ...
  ...
  .then(data => {
    ...
    .then(() => {
      ...
      .then(() => manager.waitForNotification(uuid, AuthStatusRxMessage.opcode, 15000))
      .then(data => {
        const status = new AuthStatusRxMessage(data);
        if (status.bonded !== 1) {
