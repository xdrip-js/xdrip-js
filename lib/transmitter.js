...
function control() {
  ...
  .then(data => {
    ...
    return processCalibrations(uuid, syncDate, timeMessage);
...
function processCalibrations(uuid, syncDate, timeMessage) {
  ...
  while (calibrations.length) {
    ...
    promise = processCalibrations.then(function() {
      const calibrationMessage = new CalibrateGlucoseTxMessage(
        calibration.glucose,
        timeMessage.currentTime - Math.floor((syncDate - calibration.date)/1000)
      );
