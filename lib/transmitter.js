...
function authenticate() {
  ...
  .then(data => {
    ...
    .then(() => manager.readValueAndWait(uuid, BondRequestTxMessage.opcode))
    .then(() => manager.wait(5000))
    .then(() => authenticate());
  });
}
...
function authenticateAlternate() {
  ...
  return manager.readValueAndWait(uuid)
  .then(data => console.log('received data ' + data.toString('hex')))
  // assume for now we got 0xff0601
  .then(() => {
...
//    .then(() => {
      ...
      // two possibilities here: 0xff or 0x05
      return manager.writeValueAndWaitForNotification(message.data, uuid, 0x08)
      .then(() => manager.waitForNotification(uuid, BondRequestRxMessage.opcode, 15000))
      .then(() => {
        const message = new DisconnectTxMessage();
        return manager.writeValueAndWait(message.data, UUID.CGMServiceCharacteristic.Control);
      });
