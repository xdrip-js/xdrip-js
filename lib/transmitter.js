...
async function doNext() {
  ...
  if (p.length === 2) {
    ...
    */
    parent.savePersist();
    if (this.manager.isDeviceLocallyBonded(peripheral.address.toUpperCase())) {
...
async function authenticate2() {
  ...
  while (true) {
    ...
    if (currentValueHex !== previousValueHex) {
      ...
      if (
        ((this.plugin.state !== Plugin.RequestAuth) || (data.length > 5)) &&
        this.plugin.receivedResponse(data)
      ) {
        await doNext.call(this);
      }
    }
