...
async function processBackfill() {
  ...
  if (this.backfill) {
    ...
    //this.backfill_end = (new Date() - this.activationDate.getTime()) / 1000;
    //this.backfill_start = (new Date() - this.activationDate.getTime() - 3*ONE_HOUR) / 1000;
    ...
    var backfillParser = this.parser;
    ...
    // Send the BackfillTxMessage and receive a BackfillRxMesssage. This should start trigerring callbacks on notification setup above.
    const data = await this.manager.writeValueAndWaitForNotification(backfillMessage.data, uuid.Control);
...
module.exports = class Transmitter extends EventEmitter {
  ...
  async isReady() { // TODO: perhaps change the name to onReady, or go?
    try {
      ...
      while (messages.length) {
        ...
        if (message.type=="Backfill") {
          ...
          if (message.endDate) {
            ...
            var ONE_MIN = 60 * 1000; /* ms */
            this.backfill_end = (new Date() - activationDate.getTime() - 5 * ONE_MIN)/1000;
          }
        }
        else {
      ...
      try {
        const backfill = await processBackfill.call(this);
      } catch (error) {
         debug(error);
      }
      ...
      // FIXME: better error handling, bad backfill parsing or issues shouldn't stop glucose. Glucose should have priority.
      if (this.backfill) {
        if (this.parser.validate()) {
          debug(`Parsing backfill data`);
          var backfillData = this.parser.parse();
