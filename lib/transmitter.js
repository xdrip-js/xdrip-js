...
module.exports = class Transmitter extends EventEmitter {
  constructor(id, getMessages = (() => []), altBt = false, sensorKey = null, macAddress = null) {
    ...
    this.SessionStartTxMessage = (this.g6Transmitter)
      ? require('./messages/g6/session-start-tx-message')
      : require('./messages/g5/session-start-tx-message');

    if (this.g6PlusTransmitter || this.g6AnubisTransmitter || this.g7Transmitter) {
      this.BatteryStatusRxMessage = require('./messages/g6/battery-status-rx-message');
    } else {
      this.this.BatteryStatusRxMessage = require('./messages/g5/battery-status-rx-message');
    }
  ...
  async authenticate2() {
    ...
    /* eslint-disable no-await-in-loop -- Authentication is a sequential state machine */
    while (!bonded) {
      const data = await this.manager.readValueAndWait(uuid.Authentication, null, 5);
  ...
  */
  ...
  async processBackfill() {
    ...
    if (this.backfill) {
      ...
      await this.manager.setupNotification(uuid.Backfill, notifier.notify);
      ...
      debug(`BackfillTxMessage data sent ${backfillMessage.data.toString('hex')}`);
      ...
      while (!backfillDone) {
        ...
        if (data[0] === BackfillRxMessage.opcode) {
          ...
          debug(`BackfillControlRxMessage: ${JSON.stringify(data)}`);
        } else {
          debug(`unexpected msg: ${JSON.stringify(data)}`);
        }
      }
  ...
  shouldConnect(peripheral) {
    ...
    if (this.g7Transmitter && (name.substr(0, 4) === 'DXCM')) {
      ...
      if (this.savedMacAddress && (peripheral.address === this.savedMacAddress)) {
        debug(`shouldConnect: ${peripheral.address} === ${this.savedMacAddress}(saved) => connect!`);
        this.emit('sawTransmitter', { id: this.id, rssi: peripheral.rssi, address: peripheral.address });
        return true;
      }

      debug(`shouldConnect: ${peripheral.address} !== ${this.savedMacAddress}(saved) => do not connect!`);
      return false;
    }
