...
class Plugin {
  ...
  static getInstance(password) {
    if (!Plugin.instance || Plugin.instance.context.password !== password) {
      // Log.log('KEKS-Plugin', 'Creating new instance');
  ...
  changeState(newState) {
    // Log.log("KEKS-Plugin",
    // `Changing state: ${Plugin.stateToName(this.state)} -> ${Plugin.stateToName(newState)}`);
  ...
  fill(data) {
    ...
    // Log.log("KEKS-Plugin",
    // `Expected byte size for state: ${Plugin.stateToName(this.state)} -> '
    // + '${expected} so far ${this.accumulator.length}`);
    return this.accumulator.length >= expected;
  }

  receivedData(data) {
    // Log.log("KEKS-Plugin", `Received data stream: ${Util.bytesToHex(data)}`);
    ...
    if (full) {
      ...
      if (!parsed || !this.validate(parsed)) {
        // Log.log("KEKS-Plugin", `Failed validation ${Plugin.stateToName(this.state)}`);
    ...
    switch (this.state) {
      ...
      case Plugin.ChallengeReply:
        ...
        if (!status.isAuthenticated()) {
          // Log.log("KEKS-Plugin", "Could not authenticate!");
          this.context.reset();
        ...
        // Authenticated!
        if (status.isBonded()) {
          // Log.log("KEKS-Plugin", "Full success");
      ...
      case Plugin.SendCertificate1:
        if (rep.valid()) {
          Plugin.expectedSize = rep.getSize();
      ...
      case Plugin.SendCertificate2:
        if (rep.valid()) {
          Plugin.expectedSize = rep.getSize();
          // Continue to next state
          Plugin.changeState(Plugin.SendCertificate2);
          return true;
        }
        throw new Error('InvalidParameterException: Invalid QR code');
  ...
  aNext() {
    // Log.log("KEKS-Plugin", `Processing aNext in state: ${Plugin.stateToName(this.state)}`);
    const challengeHash = Calc.calculateHash(this.context);
    const authChallengeMsg = new AuthChallengeTxMessage(challengeHash);
    const signMsg = new SignChallengeTxMessage(); // default random challenge
      ...
      case Plugin.Round3:
        ...
        return [
          ...
          Calc.getRound3Packet(this.context) ? new BlePacket(
            Calc.getRound3Packet(this.context).getHash(),
            Calc.getRound3Packet(this.context).getPublicKeyPoint1(),
            Calc.getRound3Packet(this.context).getPublicKeyPoint2(),
          ).output() : null,
        ];
  ...
  setPersistence(channel, data) {
    ...
    if (channel === 2) {
      ...
      if (!this.context.savedKey) {
        // Log.log("KEKS-Plugin", "Updating saved key from loaded persistence data");
        this.context.savedKey = data;
        return true;
      }
      return false;
    }
    // channels 3,4,6-10 require more implementation
    return false;
  }
  ...
  validate(parsedPacket) {
    this.context.packet[this.positionFromState()] = parsedPacket;
    switch (this.state) {
      ...
      default:
        // Log.log("KEKS-Plugin",
        // `Invalid state for validation: ${Plugin.stateToName(this.state)}`);
        return false;
    }
  }
  ...
  sequencePacket(packetObj) {
    ...
    if (packetObj) {
      packetBytes = new BlePacket(
        packetObj.getHash(),
        packetObj.getPublicKeyPoint1(),
        packetObj.getPublicKeyPoint2(),
      ).output();
    }
