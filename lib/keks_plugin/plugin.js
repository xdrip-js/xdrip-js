...
const debug = require('debug')('keks-plugin');
...
class Plugin {
  ...
  static getInstance(password) {
    if (!Plugin.instance || Plugin.instance.context.password !== password) {
      debug('Creating new static instance');
  ...
  changeState(newState) {
    debug(`${this.name} Changing state: ${Plugin.stateToName(this.state)} -> ${Plugin.stateToName(newState)}`);
  ...
  receivedData(data) {
    debug(`${this.name} Received data stream: ${Util.bytesToHex(data)}`);
    ...
    if (full) {
      ...
      if (!parsed || !this.validate()) {
        if (!parsed) {
          debug(`${this.name} failed to parse data stream packet`);
        } else {
          debug(`${this.name} failed to validate parsed data stream packet`);
        }
  ...
  receivedResponse(data) {
    debug(`${this.name} Received remote response: `
    ...
    if (data.length === 2 && data[0] === Config.Get.KEYCMD[0]) {
      const param = data[1];
      debug(`${this.name} Remote desires to start at Round${param + 1}`);
      if (param >= 0 && param <= 2) {
    ...
    switch (this.state) {
      ...
      case Plugin.ChallengeReply: {
        ...
        if (!status.isAuthenticated()) {
          debug(`${this.name} Could not authenticate!`);
          this.context.reset();
  ...
  aNext() {
    debug(`${this.name} Processing aNext in state: ${Plugin.stateToName(this.state)}`);
    ...
    switch (this.state) {
      case Plugin.RoundStart:
        ...
        if (this.name === 'bob') {
          debug(`${this.name} sending round1 packet`);
          return this.sequencePacket(Calc.getRound1Packet(this.context));
        }

        debug(`${this.name} sending request to start packet`);
      ...
      case Plugin.Round1: {
        ...
        if (this.name === 'bob') {
          debug(`${this.name} sending round2 packet`);
      ...
      case Plugin.Round2:
        ...
        if (this.name === 'bob') {
          debug(`${this.name} sending round3 packet`);
          return [
            this.getAuthRequestTx2().byteSequence,
            Calc.getRound3Packet(this.context) ? new BLEPacket(
              Calc.getRound3Packet(this.context).hash,
              Calc.getRound3Packet(this.context).publicKeyPoint1,
              Calc.getRound3Packet(this.context).publicKeyPoint2,
            ).output() : null,
          ];
        }
  ...
  setPersistence(channel, data) {
    ...
    if (channel === 2) {
      ...
      if (!this.context.savedKey) {
        debug(`${this.name} Updating saved key from loaded persistence data`);
  ...
  validate() {
    debug(`${this.name} validating packet round${this.positionFromState()} in state ${Plugin.stateToName(this.state)}`);
    switch (this.state) {
      ...
      case Plugin.SendCertificate2out:
        return true;
      default:
        debug(`${this.name} Invalid state for validation: ${Plugin.stateToName(this.state)}`);
        return false;
    }
  }
