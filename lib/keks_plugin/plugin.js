...
class Plugin {
  ...
  constructor(password = null, role = 'bob') {
  ...
  changeState(newState) {
    info(`${this.name} Changing state: ${this.stateToName(this.state)} -> ${this.stateToName(newState)}`);
  ...
  receivedData(data) {
    ...
    info(`${this.name} Received data (${data.length}B} in state ${this.stateToName(this.state)}`);
    debug(`${this.name} Total ${expectedBytes}B needed - have ${haveBytes}B + ${data.length}B = ${haveBytes + data.length}B`);
    debug(`${this.name} Received data ${data.length}B}: ${Util.bytesToHex(data)}`);
    ...
    if (full) {
      ...
      info(`${this.name} validated round${this.positionFromState()} packet in state ${this.stateToName(this.state)}`);
    }
    return full;
  }
  ...
  receivedResponse(data) {
    info(`${this.name} Received response in ${this.stateToName(this.state)}`);
    ...
    switch (this.state) {
      case Plugin.RoundStart:
        ...
        if (this.name === 'bob' && data.length === 2 && data[0] === Config.Get.KEYCMD[0]) {
          const param = data[1];
      ...
      case Plugin.RequestAuth:
        ...
        if (this.name === 'bob' && data[0] === AuthRequestTxMessage2.opcode) {
  ...
  verifyChallenge(data) {
    /**
     * commented this out. We should probably verify the challenge
     * even if we have a shared key?

    if (this.getSharedKey() != null) {
    ...
    */
    ...
    info(`${this.name} verifying challenge ${Util.bytesToHex(this.context.challenge)}`);
  ...
  receivedData2() { return false; }

  receivedData3() { return false; }

  getAuthRequestTx2() {
    this.lastAuthTx2 = AuthRequestTxMessage2.create(8, this.alt, this.newFwchal);
    return this.lastAuthTx2;
  }

  aNext() {
    info(`${this.name} Processing aNext in state: ${this.stateToName(this.state)}`);

    switch (this.state) {
      case Plugin.RoundStart:
        if (this.context.getRound3Packet() != null || this.getSharedKey() != null) {
          this.changeState(Plugin.RequestAuth);

          info(`${this.name} sending getAuthRequestTx2 to start Round`);
      ...
      case Plugin.Round3: {
        ...
        const authRequestTx2 = this.getAuthRequestTx2().byteSequence;
  ...
  validate() {
    switch (this.state) {
      ...
      case Plugin.SendCertificate2out:
        return true;
      default:
        info(`${this.name} Invalid state for validation: ${this.stateToName(this.state)}`);
        return false;
    }
  }
