...
class Plugin {
  ...
  constructor(password = null, role = 'bob', keyA = null, keyB = null, exponent = null) {
    ...
    if (keyA !== null) {
      this.context.keyA = KeyPair.fromPrivateBytes(Util.hexStringToByteArray(keyA, false));
    } else {
      this.context.keyA = KeyPair.generate();
    }

    if (keyB !== null) {
      this.context.keyB = KeyPair.fromPrivateBytes(Util.hexStringToByteArray(keyB, false));
    } else {
    ...
    if (exponent !== null) {
      Curve.setFixedExponent(Util.hexStringToByteArray(exponent, false));
    }
  ...
  receivedResponse(data) {
    ...
    switch (this.state) {
      ...
      case Plugin.RequestAuth:
        ...
        if (this.name === 'alice' && data[0] === AuthRequestTxMessage2.opcode) {
      ...
      case Plugin.SendKeyChallenge:
        if (data.length > 2 && data[1] === SignChallengeTxMessage.opcode) {
          // transmitter is responding to our challenge message
          return false;
