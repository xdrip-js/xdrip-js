...
class Digest {
   ...
   */
  doFinal() {
    debug(`digest doFinal: ${this.store.toString('hex')}`);
  ...
  updateDigestIncludingSize(pointOrBytes) {
    ...
    if (typeof pointOrBytes.encode === 'function' || pointOrBytes instanceof JECPoint) {
      ...
      bytes = point.getEncoded(false);
    } else {
      bytes = pointOrBytes;
    }

    const lenBuf = Util.intToByteArray(bytes.length);
    this.update(lenBuf); // each item being added needs to its length prepended

    this.update(bytes);

    return lenBuf.length + bytes.length;
  }
}
