...
class Calc {
  static getRound12Packet(context, part2 = false) {
    ...
    const party = part2 ? context.alice : context.alice; // if part2 true, may need to be contex.bob
  ...
  static validateRound3Packet(context) {
    ...
    const x3 = context.getRound1Packet().publicKeyPoint1.point;

    const g = x1.add(x2).add(x3);
    const public1 = packet.publicKeyPoint1;

    return ZKP.validateZeroKnowledgeProof(
      g,
      public1,
      packet.publicKeyPoint2,
      packet.hash,
      context.bob,
    );
  }
