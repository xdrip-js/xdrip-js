...
class Calc {
  static getRound12Packet(context, part2 = false) {
    ...
    debug(`${context.role} creating round${part2 ? 2 : 1} packet with party = ${party}`);
  ...
  static getShortSharedKey(context) {
    if (!context.savedKey) {
      ...
      if (full) {
        context.savedKey = full.slice(0, 16);
      } else {
        context.savedKey = null;
      }
    }
  ...
  static calculateHash(context) {
    ...
    let key = context.savedKey;

    if (!key) {
      key = Calc.getShortSharedKey(context);
    }
