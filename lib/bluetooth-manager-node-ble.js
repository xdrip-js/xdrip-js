...
module.exports = class BluetoothManager {
  ...
  // Replacement for scanForPeripheral + noble stateChange handling
  async startDiscovery() {
    const adapter = await this.getAdapter();
    try {
      if (!(await adapter.isDiscovering())) {
        debug('Starting discovery via node-ble');
    ...
    try {
      ...
      addresses.forEach(async (addr) => {
        ...
        try {
          ...
          debug(`Error processing discovered device ${addr}: ${err.message}`);
        }
      });
    } catch (err) {
