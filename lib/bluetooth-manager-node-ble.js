...
module.exports = class BluetoothManager {
  ...
  async initializeBluetooth() {
    ...
    try {
      this.seenDevices.clear(); // ← Reset seen devices after each watchdog restart
      // 1. Make sure the adapter is powered on
  ...
  startDiscoveryWatchdog(intervalMs = 3000, stopGapMs = 180) {
    ...
    this.discoveryWatchdog = setInterval(async () => {
      try {
        ...
        await new Promise((resolve) => setTimeout(resolve, stopGapMs));
